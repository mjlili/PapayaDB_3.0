...
public class BddQuery extends AbstractVerticle {
	...
	public void start() {
		...
		System.out.println(
				"The Server is listening for HTTP requests on port 8060 and for HTTPS requests on port 8050");
	}
	...
	private void getAllDBs(RoutingContext routingContext) {
		try {
			routingContext.response().setStatusCode(200).putHeader("content-type", "application/json")
					.end(bdd.getAllDatabases().map(Json::encodePrettily).collect(joining(", ", "[", "]")));
		} catch (IOException e) {
	...
	private void insert(RoutingContext routingContext) {
		...
		try {
			routingContext.response().setStatusCode(201).putHeader("content-type", "application/json")
					.end(bdd.insertDocumentIntoDatabase(routingContext.request().getParam("name"), routingContext.getBodyAsJson())
	...
	private void createdatabase(RoutingContext routingContext) {
		if (isAuthentified(routingContext.request())) {
			try {
				routingContext.response().setStatusCode(201).putHeader("content-type", "application/json")
						.end(bdd.createDatabase(routingContext.request().getParam("name")).map(Json::encodePrettily)
								.collect(joining(", ", "[", "]")));
	...
	private void get(RoutingContext routingContext) {
		try {
			routingContext.response().setStatusCode(201).putHeader("content-type", "application/json")
					.end(bdd.getDocumentByCriteria(routingContext.request().getParam("name"), routingContext.getBodyAsString())
							.map(Json::encodePrettily).collect(joining(", ", "[", "]")));
		} catch (IOException e) {
			routingContext.response().setStatusCode(401).end();
		}
	...
	private void drop(RoutingContext routingContext) {
		try {
			routingContext.response().setStatusCode(201).putHeader("content-type", "application/json")
					.end(bdd.dropDocumentByName(routingContext.request().getParam("name"), routingContext.getBodyAsString())
							.map(Json::encodePrettily).collect(joining(", ", "[", "]")));
		} catch (IOException e) {
			routingContext.response().setStatusCode(401).end();
		}
	...
	private void getdatabase(RoutingContext routingContext) {
		try {
			routingContext.response().setStatusCode(200).putHeader("content-type", "application/json")
					.end(bdd.getDatabase(routingContext.request().getParam("name")).map(Json::encodePrettily)
							.collect(joining(", ", "[", "]")));
		} catch (IOException e) {
	...
	private void dropdatabase(RoutingContext routingContext) {
		if (isAuthentified(routingContext.request())) {
			try {
				routingContext.response().setStatusCode(200).putHeader("content-type", "application/json")
						.end(bdd.dropDatabase(routingContext.request().getParam("name")).map(Json::encodePrettily)
								.collect(joining(", ", "[", "]")));
			} catch (IOException e) {
				routingContext.response().setStatusCode(500).putHeader("content-type", "application/json").end();
			}
